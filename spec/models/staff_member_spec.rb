...
RSpec.describe StaffMember, type: :model do
  ...
  describe "値の正規化" do
    ...
    example "emailに含まれる全角英数字記号を半角に変換" do
      member = create(:staff_member, email: "t１＠example.com")
      expect(member.email).to eq("t1@example.com")
    end

    example "email前後の全角スペースを除去" do
      member = create(:staff_member, email: "\u{3000}text@example.com\u{3000}")
      expect(member.email).to eq("text@example.com")
    end
  ...
  describe "バリデーション" do
    ...
    example "長音府を含むfamily_name_kanaは有効" do
      member = build(:staff_member, family_name_kana: "アルセーヌ")
